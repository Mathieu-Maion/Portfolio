const AVAILABLE_LANGUAGE = ["en", "fr"];
var translations = {};
var hideContactBar;
...
function contactBarEvents() {
    hideContactBar = window.innerWidth <= 900;
    if (isDevicePhone()) {
        showContactBar(true);
    } else {
        if (hideContactBar) {
...
function headerMenuEvents() {
    ...
    const as = header.getElementsByTagName('a');
    ...
    for (let a of as) {
        a.onclick = (event) => {
            if (!event.target.classList.contains("selected")) {
                ...
                setTimeout(() => { contentToShow.classList.add("animationDone"); }, 2000);
            }
        }
    }
}
...
function toggleLanguage() {
    ...
    const newLang = oldLang === 'fr' ? 'en' : 'fr';
    ...
    document.getElementById('languageIcon1').src = `images/${oldLang}.png`;
    document.getElementById('languageIcon2').src = `images/${newLang}.png`;
}
...
function showDocument(documentName, documentExtension) {
    const lang = getCurrentLanguage();
    const documentURL = `ressources/${documentName}_${lang}.${documentExtension}`;
